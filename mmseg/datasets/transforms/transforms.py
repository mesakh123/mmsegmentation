...
@TRANSFORMS.register_module()
class RandomCopyMove(BaseTransform):
    ...
    def copy_move(self, results1, results2):
        ...
        results1['img'] = img1.copy()
        
        for key in results1.get('seg_fields', []):
            results1[key] = seg1.copy()
        return results1

    def save_results(self, results):
        mix_idx = np.random.randint(0, len(self.saves))
        if np.random.rand() < 0.5:
            self.saves[mix_idx] = copy.deepcopy(results)
